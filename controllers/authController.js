...
const registerUser = async (req, res) => {
  try {
    ...
    const user = await User.create({
      ...
      email,
      password,
      role: role || 'gym_member', // Default to gym member
      phone,
      isPremium: false,
      dailyVotesRemaining: 5,
      votesResetAt: Date.now(),
...
const loginUser = async (req, res) => {
  try {
    ...
    const needsReset = user.resetDailyVotes();
    if (needsReset) {
      await user.save();
    }
...
const upgradeToPremium = async (req, res) => {
  try {
    ...
    user.isPremium = true;
    user.dailyVotesRemaining = Infinity; // Unlimited votes for premium users
